...
<div class="main-layout">
    ...
    <main class="main-content">
        ...
        <div class="results-header">
            ...
            <span class="results-count">Showing {{ count($items);  }} items</span>
        </div>
            ...
            ?>
                <div class="movie-card" 
                    ...
                    <div class="card-poster">
                        <img src="{{ $poster;  }}" alt="{{ htmlspecialchars($title);  }}" 
                             loading="lazy"
                        ...
                        <div class="card-overlay">
                            <!-- PLAY BUTTON -->
                            <button class="play-icon" title="Play" onclick="event.stopPropagation(); window.location.href='{{ $play_url;  }}'">
                                <i class="fas fa-play"></i>
                            </button>
                            
                            <!-- WATCHLIST BUTTON - FIXED FOR watchlist.php -->
                            <button class="like-icon" title="Add to list" onclick="event.stopPropagation(); toggleWatchlist({{ $item['id'];  }}, '{{ $title;  }}', '{{ $poster;  }}', {{ $item['vote_average'];  }}, '{{ $year;  }}', this)">
                                <i class="<?php echo $inWatchlist ? 'fas' : 'far'; ?> fa-heart" 
                                   style="<?php echo $inWatchlist ? 'color: #e50914;' : ''; ?>"></i>
                            </button>
                            
                            <!-- INFO BUTTON -->
                            <button class="info-icon" title="More info" onclick="event.stopPropagation(); openMovieModal({{ $item['id'];  }}, '{{ $type_param;  }}')">
                    ...
                    <div class="card-footer">
                        <div class="card-rating">
                            <span class="rating-badge">⭐ {{ number_format($item['vote_average'], 1);  }}</span>
                        </div>
    ...
    fetch(`api/get_content_details.php?id=${contentId}&type=${contentType}`)
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                const inWatchlist = {{ json_encode($watchlistIds);  }}.includes(contentId);
