...
@if(!empty($movies) && $current_page == 1 && empty($search_query) && $selected_genre == 0)
    <section class="hero-section"
        style="background-image: linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.8) 100%), url('https://image.tmdb.org/t/p/original{{ $movies[0]['backdrop_path'];  }}')">
        <div class="hero-content">
            <h2 class="hero-title">{{ htmlspecialchars($movies[0]['title']);  }}</h2>
            <p class="hero-description">{{ htmlspecialchars(substr($movies[0]['overview'], 0, 200)) . '...';  }}</p>
            <div class="hero-buttons">
                <button class="btn-play" onclick="window.location.href='play.php?id={{ $movies[0]['id'];  }}'"><i class="fas fa-play"></i> Play</button>
            </div>
        </div>
    </section>
@endif
...
<div class="main-layout">
    <aside class="sidebar">
        ...
        <div class="sidebar-search">
            <form action="" method="GET" id="searchForm">
                <div class="search-wrapper">
                    ...
                    @if(!empty($search_query))
                        <a href="{{ request()->fullUrlWithQuery([]) }}" class="clear-search"><i class="fas fa-times"></i></a>
                    @endif
                </div>
                @if($selected_genre > 0)
        ...
        @if(!empty($search_query) || $selected_genre > 0 || $filter_type != 'day')
            <a href="{{ request()->fullUrlWithQuery([]) }}" class="clear-filters-btn"><i class="fas fa-undo-alt"></i> Clear All Filters</a>
        @endif
    </aside>
                ...
                <div class="movie-card" 
                    ...
                    <div class="card-poster">
                        ...
                        <div class="card-overlay">
                            <button class="play-icon" title="Play" onclick="window.location.href='play.php?id={{ $movie['id'];  }}'"><i class="fas fa-play"></i></button>
...
<!-- FINAL TITLE FIX - SAFETY NET -->
<script>
    (function() {
        // Double-check title after everything loads
        window.addEventListener('load', function() {
            if (document.title === 'Google') {
                console.warn('Google title detected at load! Fixing...');
                document.title = "KFLIX - Watch Free Movies";
            }
        });
        
        // Check again after a short delay (for any late scripts)
        setTimeout(function() {
            if (document.title === 'Google') {
                console.warn('Google title detected after timeout! Fixing...');
                document.title = "KFLIX - Watch Free Movies";
            }
        }, 500);
    })();
