...
try {
    // Get TV Genres
    $cache_file = 'cache/tv_genres.json';
    if (file_exists($cache_file) && (time() - filemtime($cache_file) < 86400)) {
        $genres = json_decode(file_get_contents($cache_file), true);
    } else {
        $genre_response = $client->request('GET', "https://api.themoviedb.org/3/genre/tv/list?language=en-US", [
    ...
    if (!empty($search_query)) {
        $url = "https://api.themoviedb.org/3/search/tv?query=" . urlencode($search_query) . "&include_adult=false&language=en-US&page=$current_page";
        if ($selected_genre > 0) $url .= "&with_genres=$selected_genre";
    } elseif ($selected_genre > 0) {
        $url = "https://api.themoviedb.org/3/discover/tv?include_adult=false&language=en-US&page=$current_page&sort_by=$sort_by&with_genres=$selected_genre";
    } else {
        $url = "https://api.themoviedb.org/3/trending/tv/$filter_type?language=en-US&page=$current_page";
    }

    if (!empty($search_query) && $sort_by) $url .= "&sort_by=$sort_by";
...
$page_title = "TV Shows - Watch Free";
include 'layout/header.php';
?>

<?php if (!empty($tvshows) && $current_page == 1 && empty($search_query) && $selected_genre == 0): ?>
    <section class="hero-section"
        style="background-image: linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.8) 100%), url('https://image.tmdb.org/t/p/original<?php echo $tvshows[0]['backdrop_path']; ?>')">
        <div class="hero-content">
            <h2 class="hero-title"><?php echo htmlspecialchars($tvshows[0]['name']); ?></h2>
            <p class="hero-description"><?php echo htmlspecialchars(substr($tvshows[0]['overview'], 0, 200)) . '...'; ?></p>
            <div class="hero-buttons">
                <button class="btn-play" onclick="window.location.href='play.php?id=<?php echo $tvshows[0]['id']; ?>&type=tv&season=1&episode=1'">
    ...
    <aside class="sidebar">
        ...
        <div class="sidebar-search">
            <form action="" method="GET" id="searchForm">
                <div class="search-wrapper">
                    ...
                    <input type="text" name="search" class="search-input" placeholder="Search TV shows..."
        ...
        <div class="filter-section">
            ...
            <select class="sort-select" id="sortSelect">
                ...
                <option value="first_air_date.desc" <?php echo $sort_by == 'first_air_date.desc' ? 'selected' : ''; ?>>First Air Date</option>
    ...
    <main class="main-content">
        <div class="results-header">
            <h1 class="section-title">
                ...
                if (!empty($search_query)) {
                    ...
                    if ($total_results > 0) echo " <span class='result-count'>($total_results results)</span>";
                } elseif ($selected_genre > 0) {
                    $genre_name = 'Selected Genre';
                    foreach ($genres as $genre) {
                        if ($genre['id'] == $selected_genre) { $genre_name = $genre['name']; break; }
                    }
                    echo htmlspecialchars($genre_name) . " TV Shows";
                } else {
            ...
            <span class="results-count">Showing <?php echo count($tvshows); ?> TV shows</span>
        </div>
        ...
        <div class="movie-grid" id="tvGrid">
            <?php foreach ($tvshows as $show):
                ...
                $year = isset($show['first_air_date']) && !empty($show['first_air_date']) ? substr($show['first_air_date'], 0, 4) : 'N/A';
                $inWatchlist = in_array($show['id'], $watchlistIds);
            ?>
                <div class="movie-card" 
                     data-id="<?php echo $show['id']; ?>" 
                     data-title="<?php echo htmlspecialchars($show['name']); ?>"
                     data-poster="<?php echo $show['poster_path']; ?>"
                     data-rating="<?php echo $show['vote_average']; ?>"
                     data-year="<?php echo $year; ?>"
                     data-watchlist="<?php echo $inWatchlist ? '1' : '0'; ?>">
                    <div class="card-poster">
                        <img src="<?php echo $poster; ?>" alt="<?php echo htmlspecialchars($show['name']); ?>" 
                             loading="lazy"
                    ...
                    <div class="card-footer">
                        <div class="card-rating">
                            <span class="rating-badge">⭐ <?php echo number_format($show['vote_average'], 1); ?></span>
                        </div>
                        <p class="movie-title"><?php echo htmlspecialchars($show['name']); ?></p>
                        <div class="movie-meta">
                            <span class="release-year"><?php echo $year; ?></span>
                            <span class="maturity-rating"><?php echo $show['adult'] ? 'R' : 'TV-14'; ?></span>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
        ...
        <?php if (!empty($tvshows) && $total_pages > 1): ?>
